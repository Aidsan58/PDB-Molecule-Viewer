...
int main() {
    ...
    // Shader Program
    Shader ourShader("shaders/6.1.coordinateSystemsVertex.glsl", "shaders/6.1.coordinateSystemsFragment.glsl");
    ...
    unsigned char *data = stbi_load("images/tree_bark.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // render container
        glBindVertexArray(VAO);
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);        // glfw: swap buffers and poll IO events (keys pressed/released, mouse moved etc.)
